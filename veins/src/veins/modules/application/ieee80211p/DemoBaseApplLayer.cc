...
// if (DemoSafetyMessage* bsm = dynamic_cast<DemoSafetyMessage*>(wsm)) {
    if (BeaconMsg* bsm = dynamic_cast<BeaconMsg*>(wsm)) {
...
// if (DemoSafetyMessage* bsm = dynamic_cast<DemoSafetyMessage*>(wsm)) {
    if (BeaconMsg* bsm = dynamic_cast<BeaconMsg*>(wsm)) {
        receivedBSMs++;
        onBSM(bsm);
    }else if(Ack *ack = dynamic_cast<Ack*>(wsm)){ // Shajib::Changes else if condition extra added
        receivedBSMs++;
        onBSM(ack);
    }
    else if (DemoServiceAdvertisment* wsa = dynamic_cast<DemoServiceAdvertisment*>(wsm)) {
...
*/
   if (dynamic_cast<DemoServiceAdvertisment*>(msg)) {
